...
def main():
  ...
  jpeg_batch = []
  for image in images:
    jpeg_batch.append(open(image, 'rb').read())

  # Call the server to predict top k classes and probabilities, and time taken
  result = predict(
  ...
  labels = []
  # Lookup label names from imagenet indices
  ...
  for i in range(0, len(images)):
    ...
    for j in range(0, class_and_probs.shape[1]):
      print(class_and_probs[i][j])


def predict(host, port, model, batch):
  ...
  # preferred signature. A different signature can be used when creating
  # the servable saved model.
  request.model_spec.signature_name = 'predict'

  # Create proto with one field 'images'
  ...
  return stub.Predict(request, 60.0)  # 60 second timeout
